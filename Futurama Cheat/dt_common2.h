...
#define SPROP_COORD				(1<<1)	// If this is set, the float/vtor is treated like a world coordinate.
...
#define SPROP_NORMAL			(1<<5)	// If this is set, the vtor is treated like a normal (only valid for vtors)

#define SPROP_EXCLUDE			(1<<6)	// This is an exclude prop (not excludED, but it points at another prop to be excluded).

#define SPROP_XYZE				(1<<7)	// Use XYZ/Exponent encoding for vtors.
	...
	const char *ToString()
	{
		...
		case DPT_Vector:
			sprintf_s(text, sizeof(text), "(%.3f,%.3f,%.3f)",
				m_vtor[0], m_vtor[1], m_vtor[2]);
			break;
#if 0 // We can't ship this since it changes the size of DTVariant to be 20 bytes instead of 16 and that breaks MODs!!!
		case DPT_Quaternion:
			sprintf_s(text, sizeof(text), "(%.3f,%.3f,%.3f %.3f)",
				m_vtor[0], m_vtor[1], m_vtor[2], m_vtor[3]);
			break;
#endif
		case DPT_String:
			if (m_pString)
