...
int InitializeMain()
{
	g_pClient = GetInterface<HLCLient>("client_panorama.dll", "VClient");
	g_pEntList = GetInterface<CEntityList>("client_panorama.dll", "VClientEntityList");
	...
	g_pGlobalVars = (CGlobalVars*)*(PDWORD)*(PDWORD)(PatternSearch("client_panorama.dll", (PBYTE)"\x8B\x0D\x00\x00\x00\x00\xF3\x0F\x10\x49\x00\x0F\x28\xC1", "xx????xxxx?xxx", NULL, NULL) + 0x2);

	DWORD* pdwClientVMT2 = (DWORD*)*(DWORD*)g_pClient;
	g_pInput = *(CInput**)(pdwClientVMT2[16] + 0x1);
	...
	g_phkCreateMove->dwHookMethod((DWORD)CreateMoveHook, 22);
